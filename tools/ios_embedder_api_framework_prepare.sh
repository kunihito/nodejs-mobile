...
NODE_INCLUDE_PATH='./src'
V8_INCLUDE_PATH='./deps/v8/include'
...
cp $UV_INCLUDE_PATH/*.h $DST_PATH
cp -R $UV_INCLUDE_PATH/uv $DST_PATH
cp -R $UV_INCLUDE_PATH/uv $DST_PATH/uv

xcodebuild build -project $NODELIB_PROJECT_PATH/NodeMobile.xcodeproj -target "NodeMobileEmbedderApi" -configuration Release -arch arm64 -sdk "iphoneos" SYMROOT=$FRAMEWORK_TARGET_DIR
xcodebuild build -project $NODELIB_PROJECT_PATH/NodeMobile.xcodeproj -target "NodeMobileEmbedderApi" -configuration Release -arch x86_64 -sdk "iphonesimulator" SYMROOT=$FRAMEWORK_TARGET_DIR
cp -RL $FRAMEWORK_TARGET_DIR/Release-iphoneos $FRAMEWORK_TARGET_DIR/Release-universal
lipo -create $FRAMEWORK_TARGET_DIR/Release-iphoneos/NodeMobileEmbedderApi.framework/NodeMobileEmbedderApi $FRAMEWORK_TARGET_DIR/Release-iphonesimulator/NodeMobileEmbedderApi.framework/NodeMobileEmbedderApi -output $FRAMEWORK_TARGET_DIR/Release-universal/NodeMobileEmbedderApi.framework/NodeMobileEmbedderApi
